...
function PANEL:AddPlayer( ply )
	...
	item.PaintOver = function()
		if ( self.SelectedItems and !IsValid( item.Player ) ) then
		...
		if ( ply and IsValid( ply ) ) then draw.SimpleText( ply:Nick() or "", "DermaDefault", 28, 3, Color( 0, 0, 0, 255 ) ) end
	end
...
function PANEL:PopulateSubmenu( plugin, submenu, title )
	...
	for _, value in pairs( submenu ) do
		...
		button.OnSelect = function()
			RunConsoleCommand( evolve.prefix, plugin.Commands[1], unpack( self:GetParent().Tab.PlayerList:GetSelectedPlayers() ), value[2] )
...
function PANEL:OpenPluginMenu( plugin )
	...
	if ( submenu ) then
		...
		RunConsoleCommand( evolve.prefix, plugin.Commands[1], unpack( self:GetParent().Tab.PlayerList:GetSelectedPlayers() ) )
