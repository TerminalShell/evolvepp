...
include( "cl_menu_playercontrols.ev.lua" )
...
net.Receive("EV_Menu", function( len, ply )
	local loc = net.ReadString()
	if ( loc == "" ) then
		evolve:Message( "menus loaded clientside." )
		net.ReadString()
	else
		menuFile = loc
		table.insert( evolve.menus, util.Decompress( util.Base64Decode( net.ReadString() ) ) )
	end
end )
...
function MENU:Initialize()
	...
	for k, menu in pairs( evolve.menus ) do
		RunString( menu )
	end
